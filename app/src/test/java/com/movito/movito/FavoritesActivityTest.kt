...
/**
 * Unit Tests للـ FavoritesActivity
 *
 * بنختبر:
 * - Activity lifecycle
 * - Language change listener registration/unregistration
 * - Intent handling
 * - Context attachment
...
class FavoritesActivityTest {
    ...
    private lateinit var activityController: ActivityController<FavoritesActivity>
    ...
    fun `activity should be created successfully`() {
        // Act & Assert
        try {
            activityController = Robolectric.buildActivity(FavoritesActivity::class.java)
                .create()
    ...
    fun `multiple activity instances should be independent`() {
        // Act
        val controller1 = Robolectric.buildActivity(FavoritesActivity::class.java)
            .create()
        val activity1 = controller1.get()

        val controller2 = Robolectric.buildActivity(FavoritesActivity::class.java)
            .create()
    ...
    @Test
    fun `activity should handle restart after stop`() {
        // Arrange
        activityController = Robolectric.buildActivity(FavoritesActivity::class.java)
            .create()
            .start()
        ...
        // Assert
        assertNotNull("Activity should survive restart", activity)
        assertFalse("Activity should not be finishing", activity.isFinishing)
    }
}

/**
 * Integration Tests للـ FavoritesActivity
 */
@RunWith(RobolectricTestRunner::class)
@Config(sdk = [28], manifest = Config.NONE)
class FavoritesActivityIntegrationTest {
    ...
    fun `activity should handle background and foreground`() {
        // Arrange
        val controller = Robolectric.buildActivity(FavoritesActivity::class.java)
            .create()
            .start()
            .resume()
    ...
    fun `activity should handle configuration changes`() {
        ...
        // Act - Simulate configuration change (pause, stop, restart)
        controller.pause()
        controller.stop()
        controller.start()
        controller.resume()
    ...
    @Test
    fun `multiple lifecycle cycles should work correctly`() {
        // Arrange
        val controller = Robolectric.buildActivity(FavoritesActivity::class.java)
            .create()
        ...
        repeat(3) {
            ...
            val activity = controller.get()
            assertNotNull("Activity should exist in cycle $it", activity)
...
/**
 * Component Tests للـ FavoritesActivity
 */
@RunWith(RobolectricTestRunner::class)
@Config(sdk = [28], manifest = Config.NONE)
class FavoritesActivityComponentTest {

    @Test
    fun `activity class should be accessible`() {
        // Assert
        assertNotNull("Activity class should exist", FavoritesActivity::class.java)
    }
    ...
    fun `activity should extend ComponentActivity`() {
        ...
        assertTrue(
            ...
            ComponentActivity::class.java.isAssignableFrom(FavoritesActivity::class.java)
        )
    }

    @Test
    fun `activity should be instantiable`() {
        val activityClass = FavoritesActivity::class.java
