...
/**
 * Unit Tests للـ SettingsActivity
 *
 * بنختبر:
 * - Activity lifecycle
 * - Intent handling
 * - Context attachment
 * - Activity creation
 * - Multiple ViewModels initialization (AuthViewModel, ThemeViewModel, LanguageViewModel)
 * - LanguageChangeObserver registration/unregistration
 * - Language change listener behavior
 * - Notifications state handling
 * - Authentication state handling
 *
 * ملاحظات مهمة:
 * - الـ Activity بتستخدم 3 ViewModels
 * - بتسجل listener في LanguageChangeObserver
 * - بتتعامل مع NotificationManagerCompat
 * - بتراقب AuthState وتعمل navigation للـ SignIn
 */
...
class SettingsActivityTest {
    ...
    @Test
    fun `activity should initialize without savedInstanceState`() {
        // Act
        activityController = Robolectric.buildActivity(SettingsActivity::class.java)
            .create(null)
        ...
        // Assert
        assertNotNull("Should initialize with null savedInstanceState", activity)
    }

    @Test
    fun `activity should initialize with empty savedInstanceState`() {
        // Arrange
        ...
        // Assert
        assertNotNull("Should initialize with empty Bundle", activity)
    }
    ...
    @Test
    fun `activity should handle restart after destroy`() {
        // Arrange
        activityController = Robolectric.buildActivity(SettingsActivity::class.java)
            .create()
            .start()
        ...
        // Assert
        assertNotNull("Should handle restart", activity)
    }
    ...
    @Test
    fun `activity should handle intent with CLEAR_TOP flag`() {
        // Arrange
    ...
    @Test
    fun `activity should handle intent with NEW_TASK flag`() {
        // Arrange
        val intent = Intent(context, SettingsActivity::class.java).apply {
    ...
    @Test
    fun `activity should handle intent with combined flags`() {
        // Arrange
        val intent = Intent(context, SettingsActivity::class.java).apply {
        ...
        // Assert
        assertNotNull("Activity should handle combined flags", activity)
        assertTrue(
            "Should have CLEAR_TOP flag",
            activity.intent.flags and Intent.FLAG_ACTIVITY_CLEAR_TOP != 0
        )
        assertTrue(
            "Should have NEW_TASK flag",
            activity.intent.flags and Intent.FLAG_ACTIVITY_NEW_TASK != 0
        )
    }

    @Test
    fun `activity should handle intent with CLEAR_TASK flag`() {
        // Arrange
        val intent = Intent(context, SettingsActivity::class.java).apply {
        ...
        // Assert
        assertNotNull("Activity should handle CLEAR_TASK flag", activity)
    }
    ...
    @Test
    fun `activity context should be valid throughout lifecycle`() {
        // Act
        activityController = Robolectric.buildActivity(SettingsActivity::class.java)
            .create()
        ...
        // Assert - Check context in different lifecycle states
        assertNotNull("Context should exist after create", activity.applicationContext)

        activityController.start()
        assertNotNull("Context should exist after start", activity.applicationContext)

        activityController.resume()
        assertNotNull("Context should exist after resume", activity.applicationContext)
    }
    ...
    @Test
    fun `activity finish should set isFinishing to true`() {
        // Arrange
        activityController = Robolectric.buildActivity(SettingsActivity::class.java)
            .create()
            .start()
        ...
        // Assert
        assertTrue("Should be finishing after finish()", activity.isFinishing)
    }
    ...
    fun `multiple activity instances should be independent`() {
        // Act
        val controller1 = Robolectric.buildActivity(SettingsActivity::class.java)
            .create()
        val activity1 = controller1.get()

        val controller2 = Robolectric.buildActivity(SettingsActivity::class.java)
            .create()
    ...
    @Test
    fun `each activity instance should have independent viewmodels`() {
        // Arrange & Act
        val controller1 = Robolectric.buildActivity(SettingsActivity::class.java)
            .create()
        val activity1 = controller1.get()
    ...
    fun `each activity instance should have its own listener`() {
        ...
        assertNotNull("Activity 1 should exist", controller1.get())
        assertNotNull("Activity 2 should exist", controller2.get())

        // Cleanup - Both should cleanup their listeners
        controller1.pause().stop().destroy()
        controller2.pause().stop().destroy()

        assertTrue("Multiple instances cleaned up successfully", true)
    }
    ...
    fun `destroying activity multiple times should be safe`() {
        ...
        try {
            ...
            fail("Destroy should be safe: ${e.message}")
        }

        // Assert
        assertTrue("Should handle destroy safely", true)
    }
    ...
    @Test
    fun `activity should handle multiple start stop cycles`() {
        // Arrange
        activityController = Robolectric.buildActivity(SettingsActivity::class.java)
            .create()
        ...
        // Assert
        assertNotNull("Activity should survive multiple start/stop", activity)
    }

    // ===== اختبارات Language & Theme Scenarios =====
    ...
    @Test
    fun `activity should maintain state during pause resume`() {
        // Arrange
        activityController = Robolectric.buildActivity(SettingsActivity::class.java)
            .create()
            .start()
        ...
        // Act - Pause and resume
        activityController.pause()
        val duringPause = activity.applicationContext
        activityController.resume()
        val afterResume = activity.applicationContext

        // Assert
        assertNotNull("Context should exist during pause", duringPause)
        assertNotNull("Context should exist after resume", afterResume)
        assertSame("Context should be the same", duringPause, afterResume)
    }
}

/**
 * Integration Tests للـ SettingsActivity
 *
...
class SettingsActivityIntegrationTest {
    ...
    @Test
    fun `activity should handle intent with multiple flags`() {
        // Arrange
        val intent = Intent(context, SettingsActivity::class.java).apply {
            addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_NEW_TASK)
        }
        ...
        // Assert
        assertNotNull("Activity should handle multiple flags", activity)
        assertTrue(
            "Should have CLEAR_TOP flag",
            activity.intent.flags and Intent.FLAG_ACTIVITY_CLEAR_TOP != 0
        )
    ...
    @Test
    fun `activity should handle rapid navigation scenarios`() {
        // Simulate user rapidly opening and closing settings
        repeat(5) {
            val controller = Robolectric.buildActivity(SettingsActivity::class.java)
                .create()
                .start()
        ...
        assertTrue("Should handle rapid navigation", true)
    }
}

/**
 * Component Tests للـ SettingsActivity
 */
@RunWith(RobolectricTestRunner::class)
@Config(sdk = [28], manifest = Config.NONE)
class SettingsActivityComponentTest {
    ...
    fun `activity should be instantiable`() {
        ...
        val activityClass = SettingsActivity::class.java
    ...
    fun `activity should have public constructor`() {
        // Act & Assert
        try {
            val constructor = SettingsActivity::class.java.getDeclaredConstructor()
